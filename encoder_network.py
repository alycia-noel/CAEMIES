...
class encoder_network(nn.Module):
    def __init__(self):
        ...
        self.feature_map = nn.Sequential(nn.Conv2d(256, 64 * 4, kernel_size=1, stride=1),
    ...
    def train_and_val(self, trainLoader, validLoader, cipherLoader, cipherValidLoader, batch_size, lr, device, epochs=10):
        ...
        optimizer = optim.Adam(self.parameters(), lr = 0.01, betas=(.5, .999))
...
encryption_network.train_and_val(train_loader, val_loader, cipher_loader, cipher_val_loader, batch_size=batch_size, lr=learning_rate, device=device, epochs=5) 
