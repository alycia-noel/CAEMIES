...
ae_plane_benign_path = '/Users/huymai/Datasets/executables/Plane_Full_AE'
...
ae_plane_benign_files =  ExecutableDataset(transform_exec, ae_plane_benign_path, all='Benign')
ae_plane_benign_split = train_val_test_split(ae_plane_benign_files, splits=[train_split, val_split, test_split])

ae_plane_benign_train_set = ae_plane_benign_split['first']
ae_plane_benign_val_set = ae_plane_benign_split['second']
ae_plane_benign_test_set = ae_plane_benign_split['third']

#============================================================================================================

# PUT SETS INTO DATALOADERS
plane_train_dataset = plane_corrupt_train_set + plane_benign_train_set + ae_plane_benign_train_set
plane_val_dataset = plane_corrupt_val_set + plane_benign_val_set + ae_plane_benign_val_set
plane_test_dataset = plane_corrupt_test_set + plane_benign_test_set + ae_plane_benign_test_set
...
# MODEL
print('=======ADVERSARIAL TRAINING ON PLANE=======')

# Instantiation
adv_plane_model = StackedNetwork(
...
adv_plane_model.load_state_dict(torch.load(pretrained_stacked_path))

# Fine-tune model and freeze encoder
train_st_percentages, val_st_percentages, train_losses, val_losses \
    =  adv_plane_model.train_and_val(train_loader, val_loader, epochs=N_epochs, batch_size=batch_size, lr=learning_rate, momentum=momentum, freeze=1)

# Test model on test set
adv_plane_model.test(test_loader)

# Save transfer plane model
adv_plane_model_path = './saved_models/pretrained_adv_plane.pt'
adv_plane_model.save_model(adv_plane_model_path)
#============================================================================================================

# RESULTS
accuracies_path = './results/stackedaccuraciesplaneadv.png'
losses_path = './results/stackedlossesplaneadv.png'

# Stacked Accuracies over N epochs
plt.figure(1)
plt.title('Stacked Network Accuracies (Plane Adversarial)')
...
plt.figure(2)
plt.title('Stacked Network Losses (Plane Adversarial)')
